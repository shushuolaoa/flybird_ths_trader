...
def yhlogin(accinfos):
    return yhAccount(accinfos)
...
class yhAccount:
    ...
    def _post_init(self):
        ...
        self.summfilename='{}\\summtable.xls'.format(  self.dirname or util.locate_folder())
...
# 导出总的余额情况
    def export_data_summ(self, category: str = 'summary') -> dict:
        ...
        if util.go_to_top(self.root):
            ...
            # 整个交易界面，距离左边界距离窗口左侧距离，_l 为 193, 界面距离顶部边界距离 1152，   窗口右边界距离左边距离，Xpos 1575,   窗口顶部距离上边界，ypos  :105
            xpos -= 566
            ypos += 366 if category in self.ctx.SUMMARY_ else 688
            # 将鼠标左移，下移到 交易界面
            handle_v = user32.WindowFromPoint(ctypes.wintypes.POINT(xpos, ypos))
            ...
            if string != '':
                rtn = util.normalize_summ(string, self.to_dict)
